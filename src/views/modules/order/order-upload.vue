<template>
  ...
  <el-dialog :modal="false" title="请选择操作,同时只能上传一个文件" :close-on-click-modal="false" :visible.sync="visible" width="38%"
    class="box">
    <div class="tips">
      <p>
        批量发货流程为：导出待发货订单，添加快递公司和单号后，选取该文件进而导入数据。
      </p>
      <p>
        注意：
      </p>
      <p>
        1.若存在正在退款或者已经退款或者已经部分发货的订单则无法批量发货。
      </p>
      <p>
        2.导入期间请不要对导出的订单做发货操作，否则该订单将无法批量发货。
      </p>
    </div>
    <el-upload ref="upload" v-loading="loading" :auto-upload="false" :v-loading="loading"
      ...
      :action="this.$http.adornUrl2('/order/order/exportOrderExcel')" class="upload-demo" name="orderExcelFile">
  ...
  methods: {
    exportOrder() {
      ...
      this.$confirm(this.$t('order.sureToExport'), this.$t('text.tips'), {
        ...
        this.$http({
          url: this.$http.adornUrl2('/order/order/unDeliveryOrderExcel'),
          methods: 'get',
