      ...
      <el-form @submit.native.prevent :inline="true" class="search-form" ref="searchForm" :model="searchForm"
        ...
        <div class="input-row">
          ...
          <el-form-item :label="$t('distributionMsg.serialNumber') + ':'">
            <el-input type="text" v-model="searchForm.merchantOrderId"
              :placeholder="$t('distributionMsg.serialNumber')"></el-input>
          </el-form-item>
    ...
    <div class="main-container">
      ...
      <div class="table-con distribution-withdrawCash-table">
        <el-table ref="distributionWithdrawCashTable" :data="dataList" header-cell-class-name="table-header"
          ...
          <!-- 流水号 -->
          <el-table-column prop="merchantOrderId" width="230" :label="$t('distributionMsg.serialNumber')">
            <template slot-scope="scope">
              {{ scope.row.merchantOrderId }}
            </template>
          </el-table-column>
          <!-- 金额 -->
          <el-table-column prop="amount" :label="$t('home.amount')" sortable>
            <template slot-scope="scope">
              {{ scope.row.amount }}
            </template>
          </el-table-column>
          <!-- 提现时间 -->
          <el-table-column prop="createTime" :label="$t('distributionMsg.withdrawalTime')" sortable width="180">
            <template slot-scope="scope">
              {{ scope.row.createTime }}
            </template>
          </el-table-column>
          <!-- 提现类型 -->
          <el-table-column prop="type" :label="$t('distributionMsg.withdrawalType')">
            <template slot-scope="scope">
          ...
          <!-- 提现状态 -->
          <el-table-column prop="state" :label="$t('distributionMsg.withdrawalStatus')">
            <template slot-scope="scope">
          ...
          <!-- 更新时间 -->
          <el-table-column prop="updateTime" :label="$t('home.updateTime')" sortable width="180">
            <template slot-scope="scope">
          ...
          <!-- 操作 -->
          <el-table-column fixed="right" align="center" :label="$t('publics.operating')" width="130">
            <template slot-scope="scope">
              <div class="text-btn-con">
                <div v-if="scope.row.state === 0 &&
                  isAuth('platform:distributionWithdrawCash:update')
                  " class="default-btn text-btn" @click="updateToSuccess(scope.row.withdrawCashId)">
      ...
      <!-- 审核 -->
      <el-dialog title="提现审核" :modal="false" width="30%" :close-on-click-modal="false" :visible.sync="visible">
        <el-form ref="checkDataForm" @submit.native.prevent :model="checkDataForm" :rules="checkRule"
          class="demo-form-inline" label-width="60px">
          <el-form-item label="审核" size="mini" prop="status">
            <el-radio-group v-model="checkDataForm.status">
              <el-radio :label="1">审核通过</el-radio>
              <el-radio :label="2">不通过</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item label="备注" prop="remarks">
            <el-input type="textarea" size="small" rows="5" :placeholder="$t('publics.remark')" v-model="dataForm.remarks"
              maxlength="250" show-word-limit></el-input>
          </el-form-item>
        </el-form>
        <span slot="footer">
          <div class="default-btn primary-btn" @click="visible = false">取消</div>
          <div class="default-btn primary-btn" @click="submitCheck()">确认</div>
        </span>
      </el-dialog>
    </div>
...
export default {
  ...
  methods: {
    // 提交审核
    submitCheck() {
      this.$refs['checkDataForm'].validate((valid) => {
        if (valid) {
          if (this.isSubmit) {
            return false
          }
          this.isSubmit = true
          this.$http({
            url: this.$http.adornUrl('/platform/distributionWithdrawCash/toSuccess'),
            method: 'put',
    ...
    updateToSuccess(withdrawCashId) {
      this.checkDataForm.withdrawCashId = withdrawCashId
      this.visible = true
    },
    // 刷新回调
    refreshChange() {
