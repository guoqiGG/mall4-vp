  ...
  >
    ...
    >
      ...
      <el-form-item :label="this.$i18n.t('product.selectLanguage')" prop="selectLanguage" v-if="langItemList && langItemList.length > 1">
        <el-select v-model="curLang" multiple :placeholder="$t('tip.select')" @change='selectLang' style="width: 400px;">
          <el-option
      ...
      <el-form-item :label="$t('sys.isHidden')"  prop="hidden">
        <el-select size="small" v-model="dataForm.hidden" :placeholder="$t('user.pleaseSelect')">
          <el-option
          ...
          if (valid) {
            ...
            for (var i = 0; i < this.menuLangList.length; i++) {
              if (!this.menuLangList[i].name) {
                this.errorMsg(this.dataForm.typeList[this.dataForm.type] + this.$i18n.t('sys.NameNoNull'))
                return false
              }
            }
            this.isSubmit = true
