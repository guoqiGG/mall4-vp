    ...
    >
      <div class="sku-border">
        ...
        <div v-for="(tag, tagIndex) in skuTags" :key="tag.tagName" class="tagTree">
          <div class="tag">
            ...
            <div class="sku-background spec-name-box">
              <div class="spec-name">
                <div class="tag-item-tit">{{ $t('prodSku.skuName') }}</div>
                <div>
                  <div>
                    ...
                    <span v-if="tagIndex === 0" class="add-img">
                      <el-checkbox v-model="addImgchecked" @change="handleAddImgchecked(tagIndex)">{{ $t('prodSku.addSpecPic') }}</el-checkbox>
                    </span>
                  ...
                  <div v-if="tagIndex === 0" class="first-tag-tips">{{ $t('prodSku.postProductTips11') }}</div>
                </div>
              </div>
            </div>
            <!-- 规格值 -->
            <div v-if="tag.tagName" class="spec-val">
              <div class="tag-item-tit">{{ $t('prodSku.skuValue') }}</div>
              <div class="tag-item-box">
                ...
                <div v-if="tagIndex === 0" class="img-tips">{{ $t('prodSku.postProductTips12') }}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="sku-background">
          <div class="default-btn add-sku-btn" @click="addskuTag()">{{$t("prodSku.addSpecifications")}}</div>
          <span v-if="!skuTags.length" class="tips">{{$t("prodSku.postProductTips13")}}</span>
  ...
  methods: {
    ...
    addskuTag () {
      if (this.skuTags.find(tag => !tag.tagName ||
        ...
        this.$message({
          message: this.$i18n.t('prodSku.completeTheAddedSpec'),
          type: 'error',
          duration: 1000
        })
        return
    ...
    changeTagName (selVal, index) {
      ...
      if (!is) {
        ...
        if (reg.test(selVal)) {
          this.$message.error(this.$i18n.t('prodSku.specName') + this.$i18n.t('prodSku.specialWordSymbolTips') + ':;')
          return
        }
        // 替换:;字符
        ...
        // 判断规格名是否超长
        if (selVal.length > 10) {
          this.$message.error(this.$i18n.t('prodSku.postProductTips11'))
          return
        }
        // 判断规格名是否重复
        if (this.skuTags.find(el => el.tagName === selVal)) {
          this.$message.error(this.$i18n.t('prodSku.skuName') + '\'' + selVal + '\' ' + this.$i18n.t('prodSku.isExistsPleaReEn'))
          return
        }
        useTag = {
          propName: selVal,
    ...
    changeTagItemName (value, tagIndex, tagItemIndex) {
      ...
      if (!this.skuTags[tagIndex].tagName) {
        // 请先选择规格
        this.$message.error(this.$i18n.t('prodSku.selectSpecFirst'))
        this.propValue = ''
        return
      }
      // value === false下拉框收起
      if (!value && this.propValue) {
        ...
        if (is) {
          ...
          if (reg.test(element)) {
            this.$message.error(this.$i18n.t('prodSku.specValue') + this.$i18n.t('prodSku.specialWordSymbolTips') + ';')
            this.propValue = ''
            return
          }
          // 替换:;字符
          ...
          if (element.length > 20) {
            this.$message.error(this.$i18n.t('prodSku.specValueCharacterLength'))
            this.propValue = ''
            return
          }
    ...
    showTagInput (tagIndex) {
      ...
      if (!this.skuTags[tagIndex].showAddTagBtn) {
        this.$message.error(
          `${this.$t('prodSku.currentlyAdded') +
          this.countSkuTagCombo +
          this.$t('prodSku.productSpecCount')}（${this.$t('shopProcess.mostAdd') +
          this.maxNumOfCombo +
          this.$t('shopProcess.group')}），${this.$t('prodSku.cannotAddUnderThisSpec')}`
        )
    ...
    checkTagItem (tag, tagIndex) {
      ...
      if (arr.indexOf(tagItem) !== -1) {
        isSame = true
        this.$message.error(this.$i18n.t('prodSku.skuValue') + '\'' + tagItem + '\' ' + this.$i18n.t('product.isExistsPleaReEn'))
        // return false
      }
      // })
      return isSame
    ...
    vaildSkuTag () {
      ...
      if (this.skuTags && this.skuTags.length) {
        this.skuTags.some(sku => {
          ...
          // 规格名不能为空
            this.$message({
              message: this.$t('prodSku.specNameNotNull'),
              type: 'error',
              duration: 1000
            })
            isValid = false
          ...
          if (sku.tagName && (!sku.tagItems.length || (sku.tagItems.length && sku.tagItems.find(item => !item.propValue)))) {
            // 规格值不能为空
            this.$message({
              message: this.$i18n.t('prodSku.specValueCannotBeEmpty'),
              type: 'error',
              duration: 1000
            })
            isValid = false
