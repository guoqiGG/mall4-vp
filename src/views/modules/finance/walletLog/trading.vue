<template>
  <div class="table-box">
    <div class="search-bar">
      <el-form @submit.native.prevent clearable :inline="true" :model="dataFrom" class="search-form" size="small">
        <div class="input-row">
          ...
          <el-form-item :label="this.$i18n.t('shop.reason') + ':'">
            <el-select style="min-width: 240px;" v-model="dataFrom.reason" clearable :placeholder="this.$i18n.t('shop.reason')">
              <el-option
  ...
  export default {
    ...
    methods: {
      ...
      getData (newData = false) {
        ...
        // 判断是否为详情页返回
        if (this.isload === '0') {
          localStorage.setItem('bbcWalletLogSearch', JSON.stringify(this.theData))
        } else {
          this.theData = JSON.parse(localStorage.getItem('bbcWalletLogSearch'))
          this.params = {
