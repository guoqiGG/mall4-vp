<template>
  <div class="mod-fina-details">
    ...
    <div class="search-bar">
      <el-form @submit.native.prevent :inline="true" class="search-form" size="small">
        <div class="input-row">
          <!-- 订单编号 -->
          <el-form-item :label="$t('order.number') + ':'">
            <el-input style="min-width: 220px;" v-model="orderId" :placeholder="$t('finaManage.pleEntOrderNum')"></el-input>
          </el-form-item>
          <!--支付方式 -->
  ...
  export default {
    data () {
      return {
        ...
        payTypeList: [this.$i18n.t('order.pointsPayment'), this.$i18n.t('order.wecProPay'), this.$i18n.t('platform.aliPay'), this.$i18n.t('order.wechatScanCodePayment'), this.$i18n.t('order.wechatH5Payment'), this.$i18n.t('order.weclAccountPay'), this.$i18n.t('order.alipayH5Payment'), this.$i18n.t('order.alipayAPPPayment'), this.$i18n.t('order.wechatAPPPayment'), this.$i18n.t('order.balancePayment'), this.$i18n.t('order.payPalPayment')], // 所有支付方式
        minAmount: null, // 交易金额
        maxAmount: null,
