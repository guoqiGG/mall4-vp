...
export default {
  data () {
    ...
    var validateRechargeAmount = (rule, value, callback) => {
      if (Number(value)<=0) {
        callback(new Error(this.$i18n.t('shop.RechargeAmount')))
      } else {
        callback()
      }
    }
    return {
      ...
      dataRule: {
        ...
        rechargeAmount:[
          {required:true, message: this.$i18n.t('shop.RechargeAmount'), trigger: 'blur'},
