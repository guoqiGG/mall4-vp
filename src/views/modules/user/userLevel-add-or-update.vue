    ...
    <el-tabs v-model="activeName" type="card">
      <el-tab-pane :label="$t('user.generalConfiguration')" name="first">
        <div class="prods-select-body">
          <el-form @submit.native.prevent :model="dataForm" label-width="120px" ref="dataForm" :rules="dataRule">
            <el-form-item :label="$t('user.levelNameCn')" prop="levelNameCn">
              <el-input
             ...
             <el-form-item :label="$t('user.levelNameEn')" prop="levelNameEn">
              <el-input
      ...
      <el-tab-pane :label="$t('user.equityAllocation')" name="third">
        <div class="prods-select-body">
          <el-form @submit.native.prevent label-width="30px">
...
export default {
  ...
  data () {
    ...
    return {
      ...
      dataRule: {
        levelNameCn: [
          { required: true, message: this.$i18n.t('user.gradeNmaeCannotEmptyCn'), trigger: 'blur' }
        ],
        img: [
  ...
  methods: {
    ...
    getUserRightsList () {
      this.$http({
        ...
        this.userRightsList = data.records.sort((a, b) => { return a.rightsId - b.rightsId })
      })
    },
    /**
     * 获取用户权益列表
    ...
    getRightsDataList () {
      this.$http({
       ...
       data.map(element=>{
        ...
        element.rightsName = lang==='zh_CN'?rightsNameCn:rightsNameEn?rightsNameEn:rightsNameCn
    ...
    checkData () {
      ...
      if (this.dataForm.levelNameCn.length < 1 || !this.dataForm.levelNameCn.trim()) {
        this.msg = this.$i18n.t('user.gradeNmaeCannotEmptyCn')
        this.isCheckSccuss = false
        return false
      }
      // 氢春豆倍率
    ...
    submitProds () {
      ...
      this.$refs['dataForm'].validate((valid) => {
        ...
        if(this.dataForm.userLevelLangList){
          this.dataForm.userLevelLangList.map((val)=>{
            if(val.lang===0)val.levelName= this.dataForm.levelNameCn
            if(val.lang===1)val.levelName= this.dataForm.levelNameEn
          })
        }else{
          this.dataForm.userLevelLangList=[
           {
              lang: 0,
              levelName: this.dataForm.levelNameCn
            },
            {
              lang: 1,
              levelName: this.dataForm.levelNameEn
            }
        ]
        }
