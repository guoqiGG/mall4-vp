  ...
  <div class="mod-shop-notice">
    <div class="search-bar">
      <el-form @submit.native.prevent :inline="true" class="search-form" ref="searchForm" :model="searchForm" size="small">
        <div class="input-row">
          <el-form-item prop="keywords" :label="$t('weixin.keyword') + ':'" >
            <el-input type="text" v-model="searchForm.keywords" :placeholder="$t('weixin.keyword') "></el-input>
          </el-form-item>
          <el-form-item prop="content" :label="$t('weixin.replyContent') + ':'">
            <el-input type="text" v-model="searchForm.content" :placeholder="$t('weixin.replyContent')"></el-input>
          </el-form-item>
          <el-form-item>
    ...
    <div class="main-container">
      <div class="operation-bar">
        <div
          ...
          v-if="isAuth('platform:imMsgBizSkills:save')"
      ...
      <div class="table-con notice-table">
        <el-table
          ...
          <el-table-column
            ...
            align="left"
            prop="keywords"
            :label="$t('weixin.keyword')"
            width="210">
            <template slot-scope="scope">
              <span class="table-cell-text">{{ scope.row.keywords }}</span>
            </template>
          </el-table-column>

          <el-table-column
            prop="content"
            :label="$t('weixin.replyContent')"
            width="380">
            <template slot-scope="scope">
              ...
              <span class="tag-text">{{ scope.row.content }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <div class="text-btn-con">
                <div
                  v-if="isAuth('platform:imMsgBizSkills:update')"
                  class="default-btn text-btn"
                  @click="addOrUpdateHandle(scope.row.id)"
                >{{$t('user.editBtn')}}</div>
                <div
                  v-if="isAuth('platform:imMsgBizSkills:delete')"
                  class="default-btn text-btn"
...
<script>
import AddOrUpdate from './im-msg-biz-skills-add-or-update'
export default {
  data () {
    return {
  ...
  methods: {
    getDataList (page, newData = false) {
      ...
      this.$http({
        url: this.$http.adornUrl('/platform/imMsgBizSkills/page'),
        method: 'get',
    ...
    deleteHandle (id) {
      ...
      this.$confirm(this.$i18n.t('seckill.isDeleOper'), this.$i18n.t('remindPop.remind'), {
        ...
        this.$http({
          url: this.$http.adornUrl('/platform/imMsgBizSkills/' + id),
          method: 'delete',
