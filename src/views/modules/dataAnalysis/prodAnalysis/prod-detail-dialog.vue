<template>
  <el-dialog
    ...
    <div class="prod-info brief">
      ...
      <div class="content">{{prodInfo.brief===''?'无':prodInfo.brief}}</div>
    </div>
      ...
      >
        <el-table-column prop="pic" :label="$t('product.pic')">
          <template slot-scope="scope">
            <ImgShow :src="scope.row.pic" :classList="['sku-img']" />
          </template>
        </el-table-column>
          ...
          if (data.skuList) {
            data.skuList.forEach((el, index) => {
              ...
              if (index === 0 && el.properties) {
                this.skuPropertiesTitleList = el.properties.split(';').map(item => item.slice(0, item.indexOf(':')))
              }
            })
          }
