  ...
  methods: {
    ...
    openWs () {
      ...
      this.imSocketTask = new WebSocket(
        this.$http.adornWsImUrl(
          ...
          Base64.encode(this.$cookie.get('bbcAuthorization_vp')) + '/' + this.userId
        )
      )
      ...
      this.imSocketTask.onmessage = (res) => {
        ...
        if (result.code === '00000' && result.data) {
          ...
          this.message = 1
          if (this.notification && this.notificationDebounce) {
            this.showMessage()
          }
          if (result.data.newMessage && this.notificationDebounce && !localStorage.getItem('bbcImBox')) {
    ...
    updateWebConfigData () {
      this.$http({
        ...
        // else if (data.bsMenuTitleClose.length >= 4) {
        //   this.fontCloseSize.fontSize = '18px'
        // }
      })
    }
