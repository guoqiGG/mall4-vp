  ...
  <div class="select-account-range">
    ...
    <el-dialog
      ...
      <el-form @submit.native.prevent :inline="true" :model="searchParam" class="demo-form-inline" size="small">
        ...
        <el-form-item>
          <div class="default-btn primary-btn" @click="searchInfo()">{{$t('product.search')}}</div>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            <span v-else-if="scope.row[propList[2]] === 2">{{$t('shop.offlineViolation')}}</span>
            <span v-else-if="scope.row[propList[2]] === 3 || scope.row[propList[2]] === 5">{{$t('shop.offlinePendingReview')}}</span>
...
export default {
  ...
  computed: {
    ...
    propList () {
      const shopProp = ['shopName', 'mobile', 'shopStatus']
    ...
    idKey () {
      return this.type === 1 ? 'shopId' : 'userId'
    }
  },
  watch: {
    dialogVisible (data) {
