  ...
  methods: {
    ...
    getCategoryList () {
      this.$http({
        ...
        this.checkLevel(this.categoryList)
      })
    },
    checkLevel (list, level) {
      for (let i = 0; i < list.length; i++) {
        ...
        if (--level > 0) {
          this.checkLevel(list[i].children, level)
        }
          ...
          prods.push(
            {
              ...
              pic: /^https?:\/\//.test(item.pic) ? item.pic : this.resourcesUrl + item.pic,
