<template>
  ...
  >
    ...
    <el-form @submit.native.prevent style="marginLeft:10px" v-if="dataForm.offlineHandleEventItemList && dataForm.offlineHandleEventItemList[0].createTime">
      <div>
    ...
    dataFormSubmit () {
      ...
      if (this.dataForm.status === 4) {
        if (!this.refuseReason) {
          this.$message.warning(this.$i18n.t('shopProcess.auditTip3'))
          return
        }
        let reg = /^\s+$/g
